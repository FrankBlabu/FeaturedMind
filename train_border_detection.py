...
def train_multilayer_convolution (data):
    ...
    with tf.name_scope ('pooling1'):
        ...
        add_variable_summary (h_conv1)
        add_variable_summary (h_pool1)
    
    with tf.name_scope ('convolution2'):
        W_conv2 = weight_variable ([5, 5, 32, 64])
        b_conv2 = bias_variable ([64])
        
        add_variable_summary (W_conv2)
        add_variable_summary (b_conv2)
    
    with tf.name_scope ('convolution_pooling_2'):
        h_conv2 = tf.nn.relu (conv2d (h_pool1, W_conv2) + b_conv2)
        h_pool2 = max_pool_2x2 (h_conv2)
        
        add_variable_summary (h_conv2)
        add_variable_summary (h_pool2)
